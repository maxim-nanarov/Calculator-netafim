...
export default function PendWithPin() {
  ...
  const [StartingPressure, setStartingPressure] = useState(0);
    ...
    if (ResultsQ !== undefined) {
      setA(
        <div className="Results">
          ...
          <label>Pressure: {ResultsQ} (Meters)</label>
        </div>
      );
    }
  }, [ResultsQ, ResultsDp]);
  ...
  const Calculate = (event: any) => {
    ...
    let sp: number = SP; //seperation from each dripper
    let k = DripperData.k; //coefficency of the dripper
    let x = DripperData.Exponent; //Exponent of the dripper
    let Pc = DripperData.Pressure; //pressure of the Pipe
    let Dlat: number; //Diameter of the Pipe
    Dlat = SPipe.Diameter;
    let Q1 = 0; //Defining the loop pressure
    sp = Number(sp); //converting the sp to number because it came as string
      ...
      <div className="MainContainer">
        <div className="Calculator">
          <div className="container">
            <h1>find the ending pressure</h1>
            <select
              name="Dripper"
              id="Dripper"
              value={SDripper}
            ...
            <input
              type="number"
              placeholder="Starting Pressure(Pin)"
              onChange={(event) => {
                setStartingPressure(Number(event.target.value));
              }}
            ></input>
            <input
              type="number"
