...
export default function FindingLengthV2() {
  ...
  const [flowVeri, setFlowVeri] = useState<number>(0);
  ...
  const Calculate = (event: any) => {
    ...
    console.log(flowVeri, Q1);
    console.log(Dp, dp);
    console.log(Q1 < flowVeri && Dp < dp);
    let fv = 1;
    while (fv < flowVeri && counter < 1000) {
      counter++;
      if (counter < 2) {
        if (Dp < Pc) {
          Qd = k * Math.pow(Dp, x);
      ...
      length = length + sp;
      console.log('Q1: ' + Q1);
      console.log('starting pressure: ' + startingPressure);
      fv = (Q1 + startingPressure) / startingPressure;
      console.log('Flow variation: ' + fv);
    }
    setResultsQ(length);
      ...
      <div className="MainContainer">
        <div className="Calculator">
          <div className="container">
            ...
            <input
              type="number"
              placeholder="Flow veriation"
              onChange={(event) => {
                setFlowVeri(Number(event.target.value));
              }}
              max="10000"
            ></input>
            <input
